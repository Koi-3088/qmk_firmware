...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [WIN_BASE] = LAYOUT_109_ansi(
        KC_ESC,   KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,   KC_F12,     KC_MUTE,    KC_PSCR,  KC_SCRL,  KC_PAUS, QK_MACRO_0, QK_MACRO_1, QK_MACRO_2, QK_MACRO_3,
        KC_GRV,   KC_1,     KC_2,     KC_3,     KC_4,     KC_5,     KC_6,     KC_7,     KC_8,     KC_9,     KC_0,     KC_MINS,  KC_EQL,     KC_BSPC,    KC_INS,   KC_HOME,  KC_PGUP,  KC_NUM,   KC_PSLS,  KC_PAST,  KC_PMNS,
        KC_TAB,   KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,     KC_Y,     KC_U,     KC_I,     KC_O,     KC_P,     KC_LBRC,  KC_RBRC,    KC_BSLS,    KC_DEL,   KC_END,   KC_PGDN,  KC_P7,    KC_P8,    KC_P9,
        KC_CAPS,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,              KC_ENT,                                   KC_P4,    KC_P5,    KC_P6,    KC_PPLS,
        KC_LSFT,            KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,              KC_RSFT,              KC_UP,              KC_P1,    KC_P2,    KC_P3,
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
	if (!host_keyboard_led_state().num_lock) {
		rgb_matrix_set_color(NUM_LOCK_INDEX, NUM_LOCK_INDICATOR_COLOR);
	}
	else if (host_keyboard_led_state().num_lock) {
		rgb_matrix_set_color(NUM_LOCK_INDEX, SOLID_COLOR_RGB);
	}
	...
	if (host_keyboard_led_state().caps_lock) {
		...
		rgb_matrix_set_color(CAPS_LOCK_INDEX, SOLID_COLOR_RGB);
	}

	rgb_matrix_set_color(16, RGB_RED); // KC_F13
	rgb_matrix_set_color(17, RGB_RED); // KC_F14
	rgb_matrix_set_color(18, RGB_RED); // KC_F15
	rgb_matrix_set_color(72, RGB_RED); // KC_F16
	return false;
}

// bad hack
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
	rgb_matrix_mode_noeeprom(RGB_MATRIX_CUSTOM_BACKLIT_SPLASH);
	switch (keycode) {
		case QK_MACRO_0: case QK_MACRO_1: case QK_MACRO_2: case QK_MACRO_3:
			return false;
		default:
